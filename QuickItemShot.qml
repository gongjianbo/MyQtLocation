...
Item {
    ...
    // 尺寸改变时，避免超出范围
    onWidthChanged: {
        if(control.visible&&(shot_area.x+shot_area.width>control.width)){
            shot_area.width=control.width-shot_area.x;
        }
    }
    onHeightChanged: {
        if(control.visible&&(shot_area.y+shot_area.height>control.height)){
    ...
    onVisibleChanged: {
        if(control.visible){
            shot_area.x=5;
            shot_area.y=5;
            shot_area.width=control.width-10;
            shot_area.height=control.height-10;
        }
    }
